...
export interface ReflectionConfig {
  ...
  /** 是否启用自动重写 */
  autoRewrite: boolean
  /** 主角名（per-work）：接 project/kb 配置，消硬编码；未配置时检测层用默认 '林深' */
  ...
  llm?: {
    ...
    /** 最大输出 tokens，默认 8192 */
    maxTokens: number
    /** 内容截取长度（前N字送LLM），默认 3000 */
    contentTruncation: number
    /** 前文截取长度（末尾N字），默认 300 */
    prevContentTruncation: number
  }
}
...
export const DEFAULT_REFLECTION_CONFIG: ReflectionConfig = {
  ...
  autoRewrite: true,
  llm: {
    temperature: DEFAULT_LLM_CONFIG.reflection.temperature,
    maxTokens: DEFAULT_LLM_CONFIG.reflection.maxTokens,
    contentTruncation: DEFAULT_LLM_CONFIG.reflection.contentTruncation,
    prevContentTruncation: DEFAULT_LLM_CONFIG.reflection.prevContentTruncation,
  },
}
