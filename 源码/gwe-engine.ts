...
export class GWEEngine {
   ...
   * 运行文本检测（审稿核心）
   */
  check(text: string, context?: WritingContext, targetWords?: number): CheckResult {
    const merged = this.ensureMerged();
    const ctx = context ?? this.context ?? null;
    const result = check(text, ctx, merged, targetWords);
  ...
  private emit(event: EngineEventType, payload?: unknown): void {
    ...
    this.listeners.get(event)?.forEach((fn) => {
      try {
        ...
        logError('GWE', 'Event listener error:', e);
      }
    });
  }
