...
// ============================================================
// ② 反向闭环：CLI 侧项目路径解析（重实现，不 import 检测工具树以保双树隔离）
// 逻辑对齐 检测工具/project-config.ts:getProjectPath，但仅取核心：读 .inkweave/project.json 的 activeProject/path
// ============================================================
function resolveCliProjectPath(projectName?: string): string {
  try {
    const cfgPath = path.join(process.cwd(), '.inkweave', 'project.json')
    if (fs.existsSync(cfgPath)) {
      const cfg = JSON.parse(fs.readFileSync(cfgPath, 'utf-8'))
      const name = projectName || cfg.activeProject
      if (name && cfg.projects && cfg.projects[name]) {
        return path.join(process.cwd(), cfg.projects[name].path)
      }
    }
  } catch {
    // 静默降级到 cwd
  }
  // fallback：当前工作目录即项目根（.inkweave 在 cwd 时 章节/ 也在 cwd 下）
  return process.cwd()
}
...
async function runWrite(args: string[]): Promise<void> {
  ...
  const agent = await WritingAgent.create({}, llm);
  agent.setProjectPath(resolveCliProjectPath(projectName));
