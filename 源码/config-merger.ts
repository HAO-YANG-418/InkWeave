...
import { DEFAULT_RADAR_WEIGHTS } from './types';
import { loadKbDefaultThresholds } from './kb-thresholds';
import { getAllNodes, getNodeOption } from './node-registry';

// ③ 阶段二 落点 A：KB 标定值作默认值来源（best-effort，缺失回退 DEFAULT_THRESHOLDS）
const KB_DEFAULT_THRESHOLDS = loadKbDefaultThresholds();
...
export function mergeConfig(options: MergeOptions): MergedConfig {
  ...
  // ---- 第1层：默认值 ----
  let thresholds: Thresholds = { ...KB_DEFAULT_THRESHOLDS };
