...
export class WritingAgent {
  ...
  /** 项目根目录（可选）：用于 ② 反向闭环定位 章节/*.fingerprint.json。由 CLI 入口透传。 */
  private projectPath?: string

  constructor(config?: Partial<WritingAgentConfig>, llm?: LLMProvider | null, orchestrator?: WritingOrchestrator, projectPath?: string) {
  ...
  /**
   * 从持久化的会话数据恢复 Agent 状态
   * 用于服务重启后继续之前的写作会话
   */
  restoreSessionFromData(session: WritingSession): void {
    this.session = session
    // 将上下文同步到编排器
    this.orchestrator.getWritingEngine().setContext(session.context)
  }
