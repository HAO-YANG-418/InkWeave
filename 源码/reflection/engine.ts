...
import type { LLMProvider, RuleViolation } from '../types'
import { llmJson, hasLLM } from '../llm-helper'
// ①-C D-SMOOTH — 语义顺滑层（单章顺滑度增强检测，挂 reflectAsync 进自纠闭环）
import { checkSemanticSmoothness } from '../checks/check-semantic-smoothness'
// ② LLM 语义层 — 对话碰撞 + 主角私心（生成时 async 自纠增强，补正则盲区）
import { checkDialogueSemantics } from '../checks/check-dialogue-semantics'
...
interface ContentSignals {
  ...
  sentences: number
  paragraphs: number
  shortSentenceRatio: number   // 短句(≤15字)占比
  longSentenceRatio: number    // 长句(≥30字)占比
  avgSentenceLength: number
  avgParagraphLength: number
  shortParagraphRatio: number  // 短段落(≤150字)占比
  dialogueRatio: number        // 对话内容占比
  contentLower: string
}

/** 关键词信号配置 */
interface KeywordSignalConfig {
  keywords: string[]
  expectedDensity: number  // 每千字期望命中数
  weight: number
}

/** 句式结构信号配置 */
interface SentenceSignalConfig {
  shortRatio?: number   // 期望短句占比
  longRatio?: number    // 期望长句占比
  weight: number
}

/** 对话密度信号配置 */
interface DialogueSignalConfig {
  expectedRatio: number  // 期望对话占比
  weight: number
}

/** 段落节奏信号配置 */
interface PacingSignalConfig {
  expectedParagraphs?: number     // 期望段落数
  shortParagraphRatio?: number    // 期望短段落占比
  weight: number
}

/** 意图信号配置 */
interface IntentSignalProfile {
  keyword?: KeywordSignalConfig
  sentence?: SentenceSignalConfig
  dialogue?: DialogueSignalConfig
  pacing?: PacingSignalConfig
}

/**
 * 15种意图类型的多信号预期配置
 * 权重: 关键词30% + 句式20% + 对话20% + 段落节奏30%
 */
const INTENT_SIGNAL_PROFILES: Record<string, IntentSignalProfile> = {
  climax: {
    keyword: {
      keywords: ['攻击', '爆发', '冲向', '斩', '轰', '碎', '破', '杀', '全力', '赌上', '怒吼', '血', '死', '战'],
      expectedDensity: 4,
      weight: 0.3,
    },
    sentence: {
      shortRatio: 0.5,  // 高潮需要50%短句加速
      longRatio: 0.1,   // 长句不超过10%
      weight: 0.2,
    },
    dialogue: {
      expectedRatio: 0.15,  // 少量对话，以动作描写为主
      weight: 0.2,
    },
    pacing: {
      expectedParagraphs: 25,  // 较多段落
      shortParagraphRatio: 0.6,  // 60%短段落，增强节奏
      weight: 0.3,
    },
  },
  emotional_impact: {
    keyword: {
      keywords: ['泪', '痛', '心', '颤抖', '拥抱', '告别', '牺牲', '感动', '悲伤', '绝望', '希望', '爱', '恨'],
      expectedDensity: 3,
      weight: 0.3,
    },
    sentence: {
      shortRatio: 0.3,   // 中等短句比例
      longRatio: 0.2,    // 允许一些长句渲染情绪
      weight: 0.2,
    },
    dialogue: {
      expectedRatio: 0.2,
  ...
  reveal_secret: {
    keyword: {
      keywords: ['原来', '其实', '真相', '秘密', '揭开', '发现', '终于', '难怪', '愣', '惊', '不敢相信', '竟然'],
      expectedDensity: 3,
      weight: 0.3,
    },
    sentence: {
  ...
  build_relationship: {
    keyword: {
      keywords: ['说', '问', '答', '笑', '看', '点头', '摇头', '伸手', '靠近', '信任', '友谊', '同伴', '约定'],
      expectedDensity: 5,
      weight: 0.3,
    },
    sentence: {
    ...
    dialogue: {
      expectedRatio: 0.35,  // 关系构建需要高对话密度
      weight: 0.2,
    },
    pacing: {
      expectedParagraphs: 15,
  ...
  advance_plot: {
    keyword: {
      keywords: ['接着', '于是', '随后', '之后', '然后', '最终', '来到', '离开', '前往', '到达', '发现', '得知', '决定'],
      expectedDensity: 4,
      weight: 0.3,
    },
    sentence: {
    ...
    pacing: {
      expectedParagraphs: 20,
      shortParagraphRatio: 0.35,
      weight: 0.3,
    },
  },
  create_conflict: {
    keyword: {
      keywords: ['对峙', '冷视', '不退', '不让', '凭什么', '你敢', '找死', '后果', '代价', '一旦', '生死', '赌', '威胁'],
      expectedDensity: 4,
      weight: 0.3,
    },
    sentence: {
    ...
    dialogue: {
      expectedRatio: 0.3,  // 冲突场景对话较多
      weight: 0.2,
    },
    pacing: {
      expectedParagraphs: 22,
  ...
  show_growth: {
    keyword: {
      keywords: ['突破', '明悟', '顿悟', '原来如此', '懂了', '明白了', '变化', '蜕变', '进阶', '升', '不一样', '力量', '掌握'],
      expectedDensity: 3,
      weight: 0.3,
    },
    sentence: {
  ...
  build_atmosphere: {
    keyword: {
      keywords: ['声', '光', '影', '暗', '冷', '热', '风', '雾', '静', '寂', '压迫', '压抑', '诡', '森', '沉', '弥漫'],
      expectedDensity: 5,
      weight: 0.3,
    },
    sentence: {
      shortRatio: 0.2,
      longRatio: 0.35,  // 氛围营造允许较长句子
      weight: 0.2,
    },
    dialogue: {
      expectedRatio: 0.1,  // 低对话，以描写为主
      weight: 0.2,
    },
    pacing: {
      expectedParagraphs: 15,
      shortParagraphRatio: 0.25,
      weight: 0.3,
    },
  },
  plant_foreshadow: {
    keyword: {
      keywords: ['奇怪', '不对劲', '隐约', '莫名', '古怪', '异样', '异常', '说不清', '微妙', '忽略', '细节', '注意到', '角落'],
      expectedDensity: 3,
      weight: 0.3,
    },
    sentence: {
  ...
  resolve_foreshadow: {
    keyword: {
      keywords: ['原来', '其实', '之前', '那天', '那时', '果然', '难怪', '终于明白', '回想', '当初', '印证', '对应'],
      expectedDensity: 3,
      weight: 0.3,
    },
    sentence: {
  ...
  transition: {
    keyword: {
      keywords: ['几天', '数日', '转眼', '一晃', '之后', '此后', '经过', '穿过', '翻过', '来到', '场景', '画面', '转过'],
      expectedDensity: 3,
      weight: 0.3,
    },
    sentence: {
    ...
    pacing: {
      expectedParagraphs: 12,  // 过渡章节段落较少
      shortParagraphRatio: 0.3,
      weight: 0.3,
    },
  },
  world_building: {
    keyword: {
      keywords: ['据说', '传说', '自古以来', '这片', '这座', '规则', '法则', '灵气', '能量', '叫做', '称为', '分为', '体系'],
      expectedDensity: 4,
      weight: 0.3,
    },
    sentence: {
      shortRatio: 0.2,
      longRatio: 0.35,  // 世界观说明需要长句
      weight: 0.2,
    },
    dialogue: {
      expectedRatio: 0.15,
  ...
  character_intro: {
    keyword: {
      keywords: ['穿着', '气息', '眼神', '气质', '外貌', '身姿', '模样', '一看', '显得', '给人', '走来', '踏入', '现身', '望'],
      expectedDensity: 4,
      weight: 0.3,
    },
    sentence: {
  ...
  raise_stakes: {
    keyword: {
      keywords: ['更', '越发', '越来越', '升级', '加剧', '扩大', '恶化', '蔓延', '紧迫', '来不及', '一旦', '否则', '最后', '只剩'],
      expectedDensity: 4,
      weight: 0.3,
    },
    sentence: {
  ...
  breather: {
    keyword: {
      keywords: ['日常', '闲聊', '玩笑', '轻松', '慵懒', '休息', '惬意', '宁静', '舒适', '熏', '烤', '煮', '热', '笑', '躺'],
      expectedDensity: 3,
      weight: 0.3,
    },
    sentence: {
      shortRatio: 0.2,
      longRatio: 0.3,  // 缓冲章节节奏较慢
      weight: 0.2,
    },
    dialogue: {
      expectedRatio: 0.3,  // 缓冲章节对话较多
      weight: 0.2,
    },
    pacing: {
...
export class SelfReflection {
   ...
   * LLM失败时自动降级，全部使用规则引擎
   */
  async reflectAsync(input: ReflectionInput, round = 0, checkerViolations?: RuleViolation[]): Promise<ReflectionResult> {
    // 始终先跑规则引擎，获得完整12维基准线
    const ruleResult = this.reflect(input, round, checkerViolations)

    if (!hasLLM(this.llm)) {
      return ruleResult
    }

    // ①-C D-SMOOTH：单章顺滑度 LLM 增强检测（有 LLM 才跑，无 LLM 时本函数静默返 []）
    // 结果并入 checkerViolations，经既有 hasCheckerError 逻辑逼生成自纠；异常降级跳过
    let smoothnessViolations: RuleViolation[] = []
    try {
      smoothnessViolations = await checkSemanticSmoothness(input.content, this.llm)
    } catch (e) {
      logWarn('Reflection', 'D-SMOOTH 顺滑度检测异常，降级跳过', e)
    }
    // ② LLM 语义层：对话碰撞 + 主角私心（补正则盲区，无 LLM 时静默返 []；异常降级跳过）
    let semanticViolations: RuleViolation[] = []
    try {
      semanticViolations = await checkDialogueSemantics(input.content, this.llm, undefined, this.config.protagonistName)
    } catch (e) {
    ...
    // 尝试LLM语义评估（v13.1: 添加错误防护，LLM异常时降级到规则引擎）
    let llmResult: Awaited<ReturnType<typeof this.evaluateSemanticWithLLM>> = null
    try {
      llmResult = await this.evaluateSemanticWithLLM(input)
    } catch (e) {
      logWarn('Reflection', 'LLM语义评估异常，降级到规则引擎', e)
      return ruleResult
    }
    if (!llmResult) {
      return ruleResult
    ...
    let allConcerns = [...llmConcerns, ...structConcerns]

    // —— R3 接线（混合分支，与 reflect() 同源）：把 checker error 合成 concern 并入 allConcerns ——
    let hasCheckerError = false
    if (mergedViolations.length) {
      const errs = mergedViolations.filter(v => v.severity === 'error')
      if (errs.length) {
        hasCheckerError = true
        allConcerns = [...allConcerns, ...errs.map(v => ({
          dimension: 'rule_check' as QualityDimension,
          severity: 0.9,
          description: `[检测期违规 ${v.ruleId}] ${v.message}`,
          suggestion: v.suggestion ?? '按检测建议修正',
    ...
    // —— ② LLM 语义层（warning 级）：把语义层 warning 并入选定的 concerns，驱动自纠但不硬拦 ——
    // 仅针对本次新增的语义层违规（dialogue_collision_llm / protagonist_stakes_llm），不影响 smoothness 既有行为
...
只输出纯JSON，以 { 开头，以 } 结尾。`
  ...
  private async evaluateSemanticWithLLM(input: ReflectionInput): Promise<{
    ...
    const llmConfig = this.config.llm ?? DEFAULT_REFLECTION_CONFIG.llm!

    const prevContent = input.previousContent
      ? `\n【前一章结尾（用于连续性判断）】\n${input.previousContent.slice(-llmConfig.prevContentTruncation)}`
      : ''
...
【章节内容】
${input.content.slice(0, llmConfig.contentTruncation)}
...
{"dimensions":[{"dimension":"维度key","score":0.80,"issue":"具体问题（引用原文）","suggestion":"改进建议","location":"相关位置"}],"highlights":["亮点"],"overallComment":"总评（1-2句话）"}`
    ...
    ], { temperature: llmConfig.temperature, maxTokens: llmConfig.maxTokens })
   ...
   *   3. 对话密度（20%权重）
   *   4. 段落节奏（30%权重）
   * 类似 v8.x 章节类型检测的升级路径，准确率从 ~50% 提升至 ~87%
   */
  private checkIntentAlignment(input: ReflectionInput): QualityConcern {
    ...
    // 提取四类信号
    const signals = this.extractAlignmentSignals(content)

    // 获取该意图的预期信号配置
    const expected = INTENT_SIGNAL_PROFILES[intent]
    if (!expected) {
      // 未知意图类型，返回低严重度默认值
      return {
        dimension: 'intent_alignment',
        severity: 0.2,
        description: `意图类型「${intent}」缺少信号配置，使用默认评估`,
        suggestion: '考虑为意图引擎补充信号配置',
      }
    }

    // 逐信号对比，计算加权分
    const scores: { signal: string; score: number; weight: number; detail: string }[] = []
    let totalWeight = 0
    let weightedScore = 0

    // 1. 关键词密度信号
    if (expected.keyword) {
      const keywordScore = this.scoreKeywordSignal(signals, expected.keyword)
      scores.push({ signal: '关键词密度', score: keywordScore, weight: expected.keyword.weight, detail: '' })
      weightedScore += keywordScore * expected.keyword.weight
      totalWeight += expected.keyword.weight
    }

    // 2. 句式结构信号
    if (expected.sentence) {
      const sentenceScore = this.scoreSentenceSignal(signals, expected.sentence)
      scores.push({ signal: '句式结构', score: sentenceScore, weight: expected.sentence.weight, detail: '' })
      weightedScore += sentenceScore * expected.sentence.weight
      totalWeight += expected.sentence.weight
    }

    // 3. 对话密度信号
    if (expected.dialogue) {
      const dialogueScore = this.scoreDialogueSignal(signals, expected.dialogue)
      scores.push({ signal: '对话密度', score: dialogueScore, weight: expected.dialogue.weight, detail: '' })
      weightedScore += dialogueScore * expected.dialogue.weight
      totalWeight += expected.dialogue.weight
    }

    // 4. 段落节奏信号
    if (expected.pacing) {
      const pacingScore = this.scorePacingSignal(signals, expected.pacing)
      scores.push({ signal: '段落节奏', score: pacingScore, weight: expected.pacing.weight, detail: '' })
      weightedScore += pacingScore * expected.pacing.weight
      totalWeight += expected.pacing.weight
    }

    const finalScore = totalWeight > 0 ? weightedScore / totalWeight : 0.5
    const severity = 1 - finalScore

    // 生成描述
    const weakSignals = scores.filter(s => s.score < 0.5)
    const description = severity > 0.3
      ? `内容与意图「${intent}」对齐度不足（${Math.round(finalScore * 100)}分）：${weakSignals.map(s => `${s.signal}偏低(${Math.round(s.score * 100)}分)`).join('；')}`
      : `内容与意图「${intent}」对齐良好（${Math.round(finalScore * 100)}分）`

    const suggestion = weakSignals.length > 0
      ? `强化${strategies.map(s => s.name).join('、')}策略的执行，重点提升：${weakSignals.map(s => s.signal).join('、')}`
      : '保持当前对齐度'
   ...
   * 提取内容的多维信号
   */
  private extractAlignmentSignals(content: string): ContentSignals {
    ...
    const shortSentenceRatio = sentenceLengths.filter(l => l <= 15).length / Math.max(sentences.length, 1)
    const longSentenceRatio = sentenceLengths.filter(l => l >= 30).length / Math.max(sentences.length, 1)
    const avgSentenceLength = sentenceLengths.reduce((s, l) => s + l, 0) / Math.max(sentences.length, 1)

    // 段落特征
    const avgParagraphLength = paragraphs.reduce((s, p) => s + p.length, 0) / Math.max(paragraphs.length, 1)
    const shortParagraphRatio = paragraphs.filter(p => p.length <= 150).length / Math.max(paragraphs.length, 1)

    // 对话密度：引号内内容占比
    const dialogueMatches = content.match(/[""「」『』"'].*?[""「」『』"']/g) || []
    const dialogueChars = dialogueMatches.reduce((s, m) => s + m.length, 0)
    const dialogueRatio = dialogueChars / Math.max(totalChars, 1)
   ...
   * 关键词信号评分
   */
  private scoreKeywordSignal(signals: ContentSignals, config: KeywordSignalConfig): number {
    const matches = config.keywords.filter(kw => signals.contentLower.includes(kw))
    const density = matches.length / Math.max(signals.totalChars / 1000, 1) // 每千字命中数
    // 期望密度：expectedDensity，实际密度超出或不足都扣分
    const ratio = Math.min(density / Math.max(config.expectedDensity, 0.5), 2.0)
   ...
   * 句式结构信号评分
   */
  private scoreSentenceSignal(signals: ContentSignals, config: SentenceSignalConfig): number {
    let score = 1.0

    // 短句比例偏差
    if (config.shortRatio !== undefined) {
      const shortDev = Math.abs(signals.shortSentenceRatio - config.shortRatio)
      score -= shortDev * 0.5
    }

    // 长句比例偏差
    if (config.longRatio !== undefined) {
      const longDev = Math.abs(signals.longSentenceRatio - config.longRatio)
      score -= longDev * 0.5
    }
   ...
   * 对话密度信号评分
   */
  private scoreDialogueSignal(signals: ContentSignals, config: DialogueSignalConfig): number {
    // 对话密度在预期范围内为满分，偏离则扣分
    const dev = Math.abs(signals.dialogueRatio - config.expectedRatio)
    return Math.max(1.0 - dev * 2, 0.1)
  }

  /**
   * 段落节奏信号评分
   */
  private scorePacingSignal(signals: ContentSignals, config: PacingSignalConfig): number {
    let score = 1.0

    // 段落数量偏差
    if (config.expectedParagraphs !== undefined) {
      const ratio = signals.paragraphs / Math.max(config.expectedParagraphs, 1)
      const dev = Math.abs(ratio - 1.0)
      score -= dev * 0.4
    }

    // 短段落比例
    if (config.shortParagraphRatio !== undefined) {
      const dev = Math.abs(signals.shortParagraphRatio - config.shortParagraphRatio)
      score -= dev * 0.4
    }
