...
export interface GWEWritingEngine {
  ...
  /** 替换整个写作上下文（用于会话恢复） */
  setContext(ctx: WritingContext): void;
  ...
  checkContent(content: string, targetWords?: number): CheckResult;
  ...
  /** ② 反向闭环：设置上章质量禁忌（生成时动态硬约束，注入 user message 动态区） */
  setPrevChapterTaboo(text: string): void;
}
...
export async function createWritingEngine(): Promise<GWEWritingEngine> {
  ...
  // ② 反向闭环：上章质量禁忌（生成时动态硬约束），由 orchestrator 每章注入
  let prevChapterTaboo = '';
  ...
  const engine: GWEWritingEngine = {
    ...
    checkContent(content: string, targetWords?: number): CheckResult {
      return checkEngine.check(content, undefined, targetWords);
    },
    ...
    // ② 反向闭环：设置上章质量禁忌（生成时动态硬约束）
    setPrevChapterTaboo(text: string): void {
      prevChapterTaboo = text || '';
    },
  };
