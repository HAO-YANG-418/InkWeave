...
class HookRotationTracker {
  ...
  /**
   * v13.1: 简单确定性伪随机数生成器（mulberry32）
   * 接受可选种子，确保测试可重现；无种子时使用 Math.random()
   */
  ...
  selectHook(
    ...
    const selected = pool[Math.floor(HookRotationTracker.seededRandom(seed) * pool.length)]
...
const INTENT_ENDING_HOOKS: Record<IntentType, EndingHookStrategy[]> = {
  ...
  emotional_impact: [
    ...
    },
    {
      ...
      // 2026-08-31 改软：原「留白更有力量」无边界，会被读成"把信息也藏了"。
      // 留白只停情绪层（不直说感受），事件/动作/因果必须写清。
      avoid: '不要让告别过于完整和圆满——情绪层留白更有力量；但事件因果、动作、谁对谁做了什么必须写清，留白不许越界到信息层'
    },
    {
      name: '物是人非',
...
export class IntentEngine {
  ...
  analyze(context: ChapterContext): IntentResult {
    try {
      ...
      logWarn('Intent', '意图分析异常，返回安全默认值', e)
      return this.buildSafeFallback(context)
    }
  }

  /**
   * v13.1: 构建安全的默认意图结果（异常时使用）
   */
  private buildSafeFallback(context: ChapterContext): IntentResult {
    const defaultDetection: IntentDetection = { type: 'advance_plot', confidence: 0.5, evidence: ['异常降级默认'] }
    return {
      intents: [defaultDetection],
      primary: defaultDetection,
      secondary: [],
      emotionalTone: { primary: 'neutral' as EmotionType, secondary: [], intensity: 0.5, valence: 0 },
      suggestedStrategies: INTENT_STRATEGIES.advance_plot,
      suggestedPacing: {
        ...
        actionRatio: 0.4,
        rationale: '异常降级，使用标准节奏',
      },
      summary: `意图分析异常，使用默认推进剧情意图。章节：${context.chapterNumber}「${context.title}」`,
    }
  }
  ...
  private adjustByStoryState(detections: IntentDetection[], context: ChapterContext): IntentDetection[] {
    ...
    // （这里简化处理，实际需要Volume信息）
    // TODO: 卷边界检测目前按固定10章切分，应从 ChapterContext 获取实际卷信息
  ...
  generatePromptStrategy(result: IntentResult, chapterNumber?: number): string {
    ...
    // 五、核心写作法则（v13.4 增强版 — 5条黄金法则，针对弱维度定向强化）
    lines.push('')
    lines.push('## 核心写作法则（必须遵守，优先级最高）')
    lines.push('')
    lines.push('【原创性】禁止套用模板，必须用具体描写代替套路词：')
    lines.push('- 替换："嘴角勾起一抹弧度/冷笑" → 抿嘴/龇牙；"眼中闪过一丝寒芒/精光" → 瞳孔收缩/眯眼；"倒吸一口凉气" → 气息一滞')
    lines.push('- 删除："脸色大变" → 血色从脸上褪尽；"这一刻，他……" → 删掉"这一刻"，立刻写动作')
    lines.push('- 删除万能修饰词："恐怖的气势""惊人的力量""毁灭性的攻击" → 用具体细节和对比替代')
    lines.push('- 删除转折信号词："然而""就在这时""他不知道的是" → 让事件本身制造转折，不需要信号词')
    lines.push('- 至少一处意料之外：新鲜的比喻 / 意外细节 / 反直觉观察')
    lines.push('')
    lines.push('【情感冲击】情绪可"显"可"示"——身体行为展现为主，也可口语化直接点名（他火了/心里一沉/怕得手抖）：')
    lines.push('- 书面总结式情绪句（"他感到一阵莫名的悲凉""陷入深深的绝望"）必须删；口语化点名（他怕了/火气顶上来）可留，配合身体反应更有冲击力')
    lines.push('- 要制造心跳时刻：在关键转折（意外发现/关系变化/艰难抉择）给读者屏住呼吸的紧张感')
    lines.push('- 高潮场景：用短句加速节奏，感官爆发密集冲击，动作升格放大')
    lines.push('')
    lines.push('【角色声音】每个出场角色必须有独特说话方式，读者不看姓名也能分辨：')
    lines.push('- 句式长短：有人只用短句（5-15字），有人爱用长句（20+字）')
    lines.push('- 用词偏好：有人习惯粗话俗语，有人习惯书面敬语，有人用本职业黑话')
    lines.push('- 语气态度：有人咄咄逼人步步紧逼，有人温吞委婉留有余地，有人阴阳怪气话中有话')
    lines.push('- 口头禅：每个重要角色至少有一个标志性表达')
    lines.push('- 禁止：所有角色用相同的"……"表示沉默，所有角色用相同的"哼""呵""啧"语气词，所有角色对话引导词只用"说""道""问"')
    lines.push('- 对话引导词必须多样化：用"冷哼""低笑""打断""接过话头""沉吟片刻""头也不抬"替代')
    lines.push('')
    lines.push('【对话质量】每个对话必须推进剧情或展现性格或制造张力，砍掉无意义对话：')
    lines.push('- 砍掉：纯功能应答（"嗯""哦""好的""知道了"），除非后面紧跟新信息')
    lines.push('- 砍掉：寒暄客套（"吃了吗""天气不错"），没有信息量的对话全删')
    lines.push('- 三句以上连续设定解释必须拆散：动作+对话交替推进，不能堆在一起')
    lines.push('- 重要对话一定要有潜台词：嘴上说的和心里想的不一样')
    lines.push('')
    lines.push('【感官丰富】每300字至少切换一种感官，禁止连续视觉描写超过3段：')
    lines.push('- 视觉：光影、颜色、形状、运动 | 听觉：声音大小、远近、质感、沉默')
    lines.push('- 触觉：温度、质地、疼痛、压力 | 嗅觉：气味来源、浓淡、变化')
    lines.push('- 味觉：血、汗、空气、食物、情绪化味觉')
    lines.push('- 关键场景（战斗/转折/高潮）必须同时使用3种以上感官')
    lines.push('- 禁止：连续3段以上只用视觉描写，禁止用"一股XX气息"万能模板代替具体感官')
