...
export interface CoolingConfig {
  ...
  /** v13.3: LLM语义检测文本截断长度（默认8000，0=不截断） */
  patternTruncation: number;
}
...
export class CoolingSystem {
  ...
  async load(currentChapter: number): Promise<void> {
    ...
    try {
      ...
      // v13.1: 加载失败时保留现有状态，仅记录警告，避免用户积累的冷却记录丢失
      logWarn('Cooling', '冷却状态持久化数据加载失败，保留现有状态');
    }
  }
  ...
  async detectPatternsAsync(text: string): Promise<Array<{
    ...
    // v13.3: 可配置截断长度，默认 8000 字，设为 0 不截断
    const maxLen = this.config.patternTruncation
    const truncated = maxLen > 0 && text.length > maxLen ? text.slice(0, maxLen) : text
...
如果没有任何套路被使用，返回 {"detected": []}`,
    ...
    const result = await llmJson<{ detected: Array<{ patternId: string; confidence: number; snippet: string }> }>(
      this.llm,
      messages,
      { temperature: DEFAULT_LLM_CONFIG.cooling.temperature, maxTokens: DEFAULT_LLM_CONFIG.cooling.maxTokens, timeoutMs: DEFAULT_LLM_CONFIG.cooling.timeoutMs },
    );
...
如果没有任何模板被使用，返回 {"detected": []}`,
    ...
    const result = await llmJson<{ detected: Array<{ templateId: string; confidence: number; matchedSteps: string[] }> }>(
      this.llm,
      messages,
      { temperature: DEFAULT_LLM_CONFIG.coolingTemplate.temperature, maxTokens: DEFAULT_LLM_CONFIG.coolingTemplate.maxTokens, timeoutMs: DEFAULT_LLM_CONFIG.coolingTemplate.timeoutMs },
    );
