...
export function check(
  ...
  // 检查项22：开头力度检测（V3.1读者视角校准：第一句必须直接砸身体刺激）
  if (!disabledChecks.has('opening_impact')) {
    checkOpeningImpact(text, stats, thresholds, violations);
  }
...
function checkFragmentedSentences(
  ...
  for (const para of paragraphs) {
    ...
    if (!matches || matches.length < 3) continue; // 段落内少于3句不检测
    ...
    for (let i = 0; i < sentences.length; i++) {
      ...
      if (isShort && !hasTwist && !hasDialogue && !hasStrongEmotion && !isPunch) {
        ...
        if (consecutiveShort >= 3) {
          ...
          violationCount++;
          if (violationCount >= 2) return; // 最多报2个
        }
        consecutiveShort = 0;
      }
    }
...
function checkDialogueConflict(
  ...
  if (dialogues.length < 4) return; // 对话太少，不检测
  ...
  // 如果对话超过4句，但没有任何反对/否定/质疑词，报warning
...
function checkOpeningTaboos(
  ...
  if (/^(我叫|我是|他叫|他是|.*今年\d+岁|.*出生在)/.test(firstSentence) ||
      /我叫|我是.*，今年|名字叫/.test(first150.substring(0, 80))) {
    violations.push({
      ruleId: 'opening_taboos',
      ruleName: '开篇自我介绍',
  ...
  if (total >= 3 || density > 2) {
    violations.push({
      ...
      suggestion: '不要写"他嘴角露出冷笑"，写他具体做了什么：他把烟按灭在桌上，烟灰弹在对方脸上，没说话。' +
        ...
        '不要写"心中暗暗发誓"，直接写他做了什么动作/说了什么话。Show, don\'t tell。',
    });
  }
}
...
function checkSensoryBalance(
  ...
  if (text.length < 500) return; // 短章节不检测
...
function checkHookConcreteness(
  ...
  if (/^(像|如|仿佛|好似|就像|如同)/.test(lastSentence) ||
      (lastSentence.includes('像') && lastSentence.length < 20)) {
    violations.push({
