...
function checkQuestionPeriod(text: string): Violation[] {
  ...
  // ② 弱疑问：疑问代词 + 句号。
  //    【2026-09-04 修复·原实现误杀严重】原正则只数"疑问词后 20 字内有句号"就报，
  //    不看是否句末、不看是否已用问号 → 三类假阳性：
  //      a) 任指："送到哪家门口都不敲门"（哪家…都 = 无论哪家，非疑问）
  //      b) 虚指："想起来问是谁送的"（"问/知道"的宾语，非疑问）
  //      c) 已用问号："你以为那是什么天命？那是…"（问号已用对，仍被报）
  //    现改为：按句末标点切小句，只对【以句号收尾】且【非任指/非虚指】的小句报错。
  const QUESTION_WORD = /哪|什么|谁|怎么|为何|为啥/;
  // 任指/虚指豁免（命中即视为非疑问句，跳过）
  // 注意：多个正则不能用 `|` 换行拼接（JS 会当成按位或运算），必须合成单个正则
  const INDEFINITE = new RegExp([
    '(什么[^，。？！]{0,4}什么)',                                        // 什么A什么B：什么料什么味
    '(什么[^，。？！]{0,6}(都|也))',                                     // 什么都不管 / 什么也不
    '(谁[^，。？！]{0,6}(都|也))',                                       // 谁都 / 谁也不
    '(怎么[^，。？！]{0,6}(都|也))',                                     // 怎么都 / 怎么也不
    '(哪[^，。？！]{0,4}(都|也))',                                       // 哪家都 / 哪都
    '(不管|无论|任凭|凡是)',                                             // 无条件连词
    '((知道|问|问问|说|想|猜|记|看|等|待|找|挑)[^，。？！]{0,6}(谁|什么|怎么|哪))', // 虚指宾语：问是谁送的 / 等着谁去盛
  ].join('|'));

  const quoteSeg = /[““]([^””]*)[””]/g;
  while ((m = quoteSeg.exec(text)) !== null) {
    const body = m[1];
    const base = m.index;
    // 切成「小句 + 句末标点」，只检查以句号/句点收尾的
    const clauseRe = /[^。．！？!?]*[。．！？!?]/g;
    let cm: RegExpExecArray | null;
    while ((cm = clauseRe.exec(body)) !== null) {
      const c = cm[0];
      if (!/[。．]$/.test(c)) continue;   // 已用问号/叹号的正确句子跳过
      if (!QUESTION_WORD.test(c)) continue; // 无疑问词
      if (INDEFINITE.test(c)) continue;   // 任指/虚指，非疑问句
      add(`检测到引号内疑问句误用句号（${c.trim()}），`, base + cm.index);
    }
  }
  return violations;
}
