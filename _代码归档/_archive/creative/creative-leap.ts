...
请生成一个${typeInfo.name}类型的创意跳跃。`
    ...
    ], { temperature: DEFAULT_LLM_CONFIG.creativeLeap.temperature, maxTokens: DEFAULT_LLM_CONFIG.creativeLeap.maxTokens })
...
请为以上每种类型各生成一个创意跳跃。`
    ...
    const result = await llmJson<{
      ...
      { role: 'system', content: systemPrompt },
      { role: 'user', content: userPrompt },
    ], { temperature: DEFAULT_LLM_CONFIG.creativeScore.temperature, maxTokens: DEFAULT_LLM_CONFIG.creativeScore.maxTokens })
