...
export interface ContextBuildOptions {
  ...
  /** ② 反向闭环：上章质量禁忌（生成硬约束），由 orchestrator 算好后传入，注入 user message 动态区 */
  prevChapterTaboo?: string;
}
...
export function styleToPrompt(style: StyleConfig): string {
  ...
  // 2026-08-31 改软：原「描写白描为主，点到即止」——"点到即止"会被模型读成"不说清"，
  // 与好懂红线（读者零回读）直接冲突。改为精准简练的正向表达，并钉死"简≠含糊"。
  else if (style.descriptionDensity < 0.3) parts.push('描写简练，用精准名词动词撑画面（简练≠含糊：事件因果、动作链、设定逻辑必须写清，读者零回读；禁的是堆砌形容词，不是禁把话说完整）');
...
export function buildWritingMessages(
  ...
  // v14.1: 缓存命中率优化 — 动态内容单独收集，放入user message
  // DeepSeek API按前缀匹配缓存，system prompt固定则缓存命中率高
  ...
  if (opts.coolingWarnings && opts.coolingWarnings.length > 0) {
    dynamicSections.push(`【冷却系统警告 - 避免套路化】\n${opts.coolingWarnings.map(w => `- ${w}`).join('\n')}`);
    dynamicSections.push('');
  }

  // 章节衔接 → 动态
  const continuity = buildContinuityHint(context);
  if (continuity) { dynamicSections.push(continuity); dynamicSections.push(''); }

  // 伏笔提醒 → 动态
  const foreshadow = buildForeshadowHint(context);
  if (foreshadow) { dynamicSections.push(foreshadow); dynamicSections.push(''); }

  // 规则 → 静态
  const rulesPrompt = formatGlobalRules(rules);
  if (rulesPrompt) { sections.push(rulesPrompt); sections.push(''); }

  // v14.2: 角色和设定移到user message（动态区）
  // getRelevantCharacters/getRelevantSettings 基于当前章节打分，
  // 每章返回的角色/设定列表不同，放在system prompt会导致缓存失效
  const charsPrompt = formatCharacters(chars);
  if (charsPrompt) { dynamicSections.push(charsPrompt); dynamicSections.push(''); }

  const settingsPrompt = formatSettings(settings);
  if (settingsPrompt) { dynamicSections.push(settingsPrompt); dynamicSections.push(''); }

  // v8.0: Coach模式 → 动态（每章变化）
  if (capability === 'continue' || capability === 'expand') {
    ...
    if (coaching) { dynamicSections.push(coaching); dynamicSections.push(''); }
  }
  ...
  if (recentChapters.length > 0) {
    ...
    dynamicSections.push(`【前情提要】\n${recentText}`);
    dynamicSections.push('');
  }

  // ② 反向闭环：上章质量禁忌 → 动态硬约束（每章变化，放 user message 动态区）
  if (opts.prevChapterTaboo && opts.prevChapterTaboo.trim()) {
    dynamicSections.push(opts.prevChapterTaboo);
    dynamicSections.push('');
  }
  ...
  // v14.1: 动态内容（每章变化）放入user message，而非system prompt
  // 这样system prompt固定不变，DeepSeek API可缓存前缀，缓存命中率从0% → 80%+
  const dynamicContext = dynamicSections.length > 0 ? dynamicSections.join('\n') + '\n' : ''

  // 构建user message
  const userMessage = dynamicContext + buildUserMessage(context, opts);
